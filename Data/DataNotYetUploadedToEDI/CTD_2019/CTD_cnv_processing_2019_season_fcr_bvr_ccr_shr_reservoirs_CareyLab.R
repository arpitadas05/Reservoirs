...
ctdRaw <- read.ctd.sbe("080119_fcr50.cnv",            
                       columns = list(), 
                       monitor = T,
...
plotScan(ctdTrim(ctdRaw, "range",
                 parameters=list(item="scan", from=235, to=100000))) # if it was a long cast (SHR), go higher than 10000

### Trim the dataframe to include ONLY the downcast (Numbers "from = XXX," will need to be adjusted)
ctdTrimmed <- ctdTrim(ctdRaw, "range",
                      parameters=list(item="scan", from=235, to=100000))
...
pdf("080119_fcr50.pdf", width=12, height=12)
...
write_csv(data_wtr, "080119_fcr50.csv")
...
pdf("080119_fcr50_PAR.pdf", width=10, height=10)
...
write_csv(par, "080119_fcr50_PAR.csv")
...
ctdTrimmed[["metadata"]]$cruise <- "Sampling on 01-Aug-19"            

# Who took the cast?
ctdTrimmed[["metadata"]]$scientist <- "AL"                         
...
ctdTrimmed[["metadata"]]$station <- "fcr50"  
...
write.table(metadata, file = "080119_fcr50_metadata.txt", sep = "\t")
